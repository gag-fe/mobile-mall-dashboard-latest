...
import {storeReportDetailNextPageCompleteReport} from '../../actions';
const pageSize =30;
class CompleteReport extends React.Component{
  constructor(props) {
    super(props);
    ...
    onEndReached = (event) => {
      ...
      console.log(1231,event);
        ...
        debugger
        t.props.nextPageCompleteReport();
      }
  ...
  render(){
      ...
      const row = (rowData, sectionID, rowID) => {
        ...
        return (
          <div key={rowID} className="store-report-detail-row" onClick={t.props.completeReportCallback.bind(null,rowData)}>
              <Flex>
                <Flex.Item>{rowData.date}</Flex.Item>
                <Flex.Item><p className="shop-entity-name">{rowData.shopEntityName}</p></Flex.Item>
                <Flex.Item>{rowData.claimMoneyTotal}</Flex.Item>
                <Flex.Item>{rowData.claimBillTotal}</Flex.Item>
              </Flex>
          </div>
        );
      };
      return(
          <ListView ref="upReport"
            ...
            renderFooter={() => {
              if(this.props.isLoading){
               ...
               if(!this.props.hasMore&&(this.props.completeReport.rows.length<=0)){
                 return(<div style={{ padding: 30, textAlign: 'center' }}>
                       没有找到匹配的数据
                </div>)
               }
  ...
  componentWillReceiveProps(nextProps) {
    ...
    let t=this;
    debugger
    console.log('nextProps.completeReport.pageIndex==',nextProps.completeReport);
    if(nextProps.completeReport.pageIndex){
      debugger
      t.genData(nextProps.completeReport.pageIndex-1,nextProps.completeReport.rows);
      this.setState({
...
function mapStateToProps(state) {
  return {
    completeReport: state.storeReportDetail.completeReport,
    isLoading:state.storeReportDetail.completeReportLoading,
    hasMore:state.storeReportDetail.completeReportMore,
  }
}

export default connect(mapStateToProps, {
  nextPageCompleteReport:storeReportDetailNextPageCompleteReport.request,
