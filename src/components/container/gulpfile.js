...
let eslintEngine = require('eslint');
let CLIEngine = eslintEngine.CLIEngine;
...
function eslint() {
  ...
  //  })
  //debugger
  //  let report = cli.executeOnFiles(['src/pages/**/*.js'])
  //  debugger
  //  let tableFormatter = cli.getFormatter('table')
  //  debugger
  //  if (report.errorCount>1 || report.warningCount) {
...
gulp.task('pack_server', function (cb) {
  debugger
  let eslintRes = eslint()
  if (eslintRes) {
    webpack(require('./webpack.docs.babel.js'), function (err, stats) {
      ...
      console.info('###### pack_server done=============LTQ ######');
      cb();
    });
  } else {
    cb('eslint error');
...
gulp.task('stylus_component', function (cb) {
  gulp.src(['./src/components/**/*.css', './src/components/**/*.styl'])
...
gulp.task('stylus_src', function (cb) {
  gulp.src([
      './tingle/**/src/*.css',
...
gulp.task('svg_inject', ['svg_sprite'], function (cb) {
  gulp.src('index.html')
    ...
    .pipe(injectStr.before('</body>', fs.readFileSync(
...
gulp.task('reload_by_js', ['pack_server'], function () {
  reload();
...
gulp.task('reload_by_demo_css', ['stylus_src'], function () {
  reload();
})
...
gulp.task('develop', [
  ...
  gulp.watch(['src/pages/**/*.js', 'components/**/*.js'], ['reload_by_js']);

  gulp.watch('components/**/*.styl', ['reload_by_component_css']);
  gulp.watch('components/**/*.css', ['reload_by_component_css']);
  gulp.watch('src/pages/**/*.styl', ['reload_by_demo_css']);
